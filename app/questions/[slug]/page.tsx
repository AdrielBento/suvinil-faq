              ...
              <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
                {relatedQuestions.map((item) => (
                  <Button
                    ...
                    variant="outline"
                    asChild
                    className="justify-start rounded-2xl border border-border/70 bg-background px-4 py-3 text-left text-sm font-medium shadow-sm transition hover:border-primary/60 hover:shadow-md"
                  >
                    <Link href={`/questions/${item.slug}`}>{item.question}</Link>
                  </Button>
                ))}
              </div>
            </section>
